...
function StaticCanvas({ doc, viewport }: { doc: TemplateDoc; viewport: 'desktop' | 'tablet' | 'mobile' }) {
  return (
    <div
      className="relative h-fit min-h-full overflow-hidden rounded-[16px] bg-surface shadow-[0_8px_32px_rgba(14,14,16,0.12)] ring-1 ring-[#E7E5E0]"
...
export function CompareView() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col p-2 sm:p-5" role="dialog" aria-modal="true" aria-label="Compare changes" data-testid="compare-modal">
      ...
      />
      <div
        className={`relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[28px] border p-1.5 shadow-[0_16px_64px_rgba(14,14,16,0.24)] sm:p-2 ${darkMode ? 'border-white/10 bg-surface/[0.06]' : 'border-[#0E0E10]/[0.08] bg-ink/[0.06]'}`}
      >
        <div
          className={`flex min-h-0 flex-1 flex-col overflow-hidden rounded-[20px] border ${darkMode ? 'border-white/10 bg-surface-dark' : 'border-stone bg-paper shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_1px_2px_rgba(14,14,16,0.06)]'}`}
        >
          <header className={`flex shrink-0 items-start justify-between gap-4 border-b px-5 py-4 sm:px-6 ${darkMode ? 'border-white/10 bg-surface-dark-raised' : 'border-stone bg-surface'}`}>
              ...
              <div>
                ...
                <ul className="flex flex-wrap gap-1.5" data-testid="compare-changed-list">
                  {changedIds.map((id) => (
                    <li
                      key={id}
                      className={`rounded-full border px-3 py-1.5 font-mono text-[11px] font-medium ${darkMode ? 'border-white/10 bg-surface-dark text-stone' : 'border-stone bg-paper text-ink shadow-sm'}`}
                      title={id}
                    >
                      {id}
                    </li>
